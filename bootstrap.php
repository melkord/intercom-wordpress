...
function settings() {
	...
	if ( isset( $_POST['_wpnonce'] ) and wp_verify_nonce( $_POST['_wpnonce'], 'intercom-update' )
		...
		$validator = new Validator( $_POST["intercom"], function ( $x ) {
			return wp_kses( trim( $x ), array() );
		} );
		update_option( "intercom", array( "app_id"     => $validator->validAppId(),
		                                  "secret"     => $validator->validSecret(),
		                                  "enableChat" => $validator->validEnableChat()
		) );
